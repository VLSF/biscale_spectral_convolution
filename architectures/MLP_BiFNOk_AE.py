...
class MLP_BiFNOk_AE(eqx.Module):
    encoder: eqx.Module
    coeff_encoder: jnp.array
    decoder: eqx.Module
    coeff_decoder: jnp.array
    x_b: jnp.array
    ...
    def __init__(self, N_layers, N_processor, N_modes, D, J_b, N_f_b, kernel_size, n_basis, key, s1=1.0, s2=1.0, s3=1.0):
        ...
        self.coeff_encoder = random.normal(keys[1], (n_basis, N_processor*2**(D*J_b))) / jnp.sqrt(n_basis*N_processor*2**(D*J_b))
        self.coeff_decoder = random.normal(keys[2], (N_processor*2**(D*J_b), n_basis)) / jnp.sqrt(n_basis*N_processor*2**(D*J_b))
    ...
    def encode(self, u_a, x_a):
        ...
        u_b = self.coeff_encoder @ u_b.reshape(-1,)
        return u_b

    def decode(self, u_b, x_a):
        u_b = self.coeff_decoder @ u_b
        shape = [-1,] + [self.x_b.shape[2],] * (self.x_b.ndim - 2)
