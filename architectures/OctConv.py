...
class OctConv(eqx.Module):
    ...
    def __init__(self, N_layers, N_features_a, N_features_b, D, kernel_size, key, J_d, s1=1.0, s2=1.0, s3=1.0):
        ...
        keys = random.split(keys[3], 3*N_layers + 1)
        self.convs1_a = [normalize_conv(eqx.nn.Conv(D, n_processor_a, n_processor_a, kernel_size, padding=kernel_size//2, key=key), s1=s1, s2=s2) for key in keys[:N_layers]]
        self.convs2_a = [normalize_conv(eqx.nn.Conv(D, n_processor_a, n_processor_a, kernel_size, padding=kernel_size//2, key=key), s1=s1, s2=s2) for key in keys[N_layers:2*N_layers]]
        self.convs3_a = [normalize_conv(eqx.nn.Conv(D, n_processor_a, n_processor_a, kernel_size, padding=kernel_size//2, key=key), s1=s1, s2=s2) for key in keys[2*N_layers:3*N_layers]]
        
        keys = random.split(keys[-1], 3*N_layers - 1)
        ...
        self.convs3_b = [normalize_conv(eqx.nn.Conv(D, n_processor_b, n_processor_b, kernel_size, padding=kernel_size//2, key=key), s1=s1, s2=s2) for key in keys[(2*N_layers-2):(3*N_layers-3)]]
    ...
    def __call__(self, u_a, x_a, u_b, x_b):
        ...
        for i in range(len(self.a_to_b)):
            ...
            u_a += gelu(self.convs3_a[i](gelu(self.convs2_a[i](self.convs1_a[i](u_a) + u_a_))))
            if i != len(self.convs1_b):
                u_b += gelu(self.convs3_b[i](gelu(self.convs2_b[i](self.convs1_b[i](u_b) + u_b_))))
