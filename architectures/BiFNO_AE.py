...
class BiFNO_AE(eqx.Module):
    encoder: eqx.Module
    decoder: eqx.Module
    x_b: jnp.array

    def __init__(self, N_layers, N_processor, N_modes, D, N_b, N_f_b, key, s1=1.0, s2=1.0, s3=1.0):
        keys = random.split(key, 3)
        self.encoder = BiFNO(N_layers, [D + 1, N_processor, 1], [N_f_b, N_processor, 1], N_modes, D, keys[0], s1=s1, s2=s2, s3=s3)
        self.decoder = BiFNO(N_layers, [D, N_processor, 1], [N_f_b + 1, N_processor, 1], N_modes, D, keys[1], s1=s1, s2=s2, s3=s3)
