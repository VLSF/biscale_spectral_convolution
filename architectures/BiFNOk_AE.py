...
class BiFNOk_AE(eqx.Module):
    encoder: eqx.Module
    decoder: eqx.Module
    x_b: jnp.array

    def __init__(self, N_layers, N_processor, N_modes, D, N_b, N_f_b, kernel_size, key, s1=1.0, s2=1.0, s3=1.0):
        keys = random.split(key, 3)
        self.encoder = BiFNOk(N_layers, [D + 1, N_processor, 1], [N_f_b, N_processor, 1], N_modes, D,  kernel_size, keys[0], s1=s1, s2=s2, s3=s3)
        self.decoder = BiFNOk(N_layers, [D, N_processor, 1], [N_f_b + 1, N_processor, 1], N_modes, D,  kernel_size, keys[1], s1=s1, s2=s2, s3=s3)
        self.x_b = random.normal(keys[2], [2,] + [N_f_b,] + [N_b,]*D)

    def encode(self, u_a, x_a):
        return self.encoder(u_a, x_a, None, self.x_b[0])[1]

    def decode(self, u_b, x_a):
        return self.decoder(None, x_a, u_b, self.x_b[1])[0]
