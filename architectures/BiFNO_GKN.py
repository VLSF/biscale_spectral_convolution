...
class BiFNO_GKN(eqx.Module):
    ...
    def __init__(self, N_layers, N_features_a, N_features_b, N_modes, D, GKN_proc, GKN_layers, key, s1=1.0, s2=1.0, s3=1.0):
        ...
        self.b_to_a = [GKN([2*n_processor_b + 2*D, GKN_proc], GKN_layers, key, s1=s1, s2=s2) for key in random.split(keys[-2], N_layers)]
        self.a_to_b = [GKN([2*n_processor_a + 2*D, GKN_proc], GKN_layers, key, s1=s1, s2=s2) for key in random.split(keys[-1], N_layers)]
