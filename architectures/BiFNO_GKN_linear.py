...
class GKN(eqx.Module):
    ...
    def __call__(self, u, x0, x1, ind0, ind1):
        ...
        w = jnp.concatenate([x0_[:, ind0], x1_[:, ind1]], axis=0)
        w = self.convs[0](w)
...
class BiFNO_GKN_linear(eqx.Module):
    FNO_a: eqx.Module
    FNO_b: eqx.Module
    b_to_a: list
    a_to_b: list
    ...
    def __init__(self, N_layers, N_features_a, N_features_b, N_modes, D, GKN_proc, GKN_layers, key, s1=1.0, s2=1.0, s3=1.0):
        ...
        self.b_to_a = [GKN([2*D, GKN_proc], GKN_layers, key, s1=s1, s2=s2) for key in random.split(keys[-2], N_layers)]
        self.a_to_b = [GKN([2*D, GKN_proc], GKN_layers, key, s1=s1, s2=s2) for key in random.split(keys[-1], N_layers)]
        self.b_to_a_ = [normalize_conv(eqx.nn.Conv(D, n_processor_b, n_processor_a, 1, key=key), s1=s1, s2=s2) for key in random.split(keys[-3], N_layers)]
        self.a_to_b_ = [normalize_conv(eqx.nn.Conv(D, n_processor_a, n_processor_b, 1, key=key), s1=s1, s2=s2) for key in random.split(keys[-4], N_layers)]

    def __call__(self, u_a, x_a, u_b, x_b, ind_a, ind_b):
        if u_a is None:
            u_a = x_a
        else:
        ...
        for i in range(self.FNO_a.A.shape[0]):
            u_a_, u_b_ = self.b_to_a_[i](self.b_to_a[i](u_b, x_b, x_a, ind_b, ind_a)), self.a_to_b_[i](self.a_to_b[i](u_a, x_a, x_b, ind_a, ind_b))
