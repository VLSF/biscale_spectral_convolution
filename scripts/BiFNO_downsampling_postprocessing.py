    ...
    for i in range(len(Data)):
        ...
        if args['J_a'] != J_min:
            ...
            for j in resolution_:
                ...
                D = targets.ndim - 2
                targets = BiFNO_train.normalise_field(BiFNO_train.subsample_field(targets, j, D), D)
                perm = random.permutation(keys[0], targets.shape[0])
                targets = targets[perm]
                
                if args['features_input'] == 'a':
                    features_a = BiFNO_train.normalise_field(BiFNO_train.subsample_field(data['features'], j, D), D)[perm]
                    features_b = None
                else:
                    features_a = None
                    features_b = BiFNO_train.normalise_field(BiFNO_train.subsample_field(data['features'], args['J_b'], D), D)[perm]
                
                coordinates_a = BiFNO_train.subsample_field(np.expand_dims(data['coordinates'], 0), j, D)[0]
                if args['Nx_b'] != 0:
