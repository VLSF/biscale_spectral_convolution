...
from architectures import OctConv_c
    ...
    args = {
       ...
       "-results_path": {
        ...
        "-kernel_size": {
            "default": 3,
            "type": int,
            "help": "kernel size of convolution used in channel b"
        },
        "-optim": {
            "default": 'adam',
            "type": str,
...
if __name__ == "__main__":
    ...
    J_d = abs(args["J_a"] - args["J_b"])
    kernel_size = args["kernel_size"]
    model = OctConv_c.OctConv(N_layers, N_features_a, N_features_b, D, kernel_size, keys[1], J_d, s1=args['s1'], s2=0, s3=args['s3'])
    ...
    for nn_ in nn:
        ...
        rel_errors = scan(compute_errors, [model, features_a, coordinates_a, features_b, coordinates_b, targets], train_ind)[1]
        train_rel_errors.append(jnp.mean(rel_errors))
    ...
    train_rel_errors = jnp.array(train_rel_errors)
    val_rel_errors = jnp.nan_to_num(val_rel_errors, nan=jnp.inf)
    train_rel_errors = jnp.nan_to_num(train_rel_errors, nan=jnp.inf)
    ...
    train_rel_errors_ = jnp.mean(scan(compute_errors, [models[best_n], features_a, coordinates_a, features_b, coordinates_b, targets], train_ind)[1])
    val_rel_errors_ = jnp.mean(scan(compute_errors, [models[best_n], features_a, coordinates_a, features_b, coordinates_b, targets], val_ind)[1])
    test_rel_errors_ = jnp.mean(scan(compute_errors, [models[best_n], features_a, coordinates_a, features_b, coordinates_b, targets], test_ind)[1])
    data = "\n" + ",".join([str(args[key]) for key in args.keys()])
    data += f",{exp_hash},{history[-1]},{model_size},{training_times[best_n]},{train_rel_errors_},{test_rel_errors_},{val_rel_errors_},{best_n}"
                                
    with open(f'{args["results_path"]}/results.csv', "a") as f:
        f.write(data)
                                
    jnp.savez(f'{args["results_path"]}/metrics_{exp_hash}.npz', rel_errors=rel_errors, history=history, permutation=perm, val_rel_errors=val_rel_errors, train_rel_errors=train_rel_errors)