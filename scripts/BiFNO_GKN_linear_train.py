...
from architectures import BiFNO_GKN_linear
...
def compute_errors(carry, ind, ind0, ind1):
    ...
    if features_a is None:
        prediction = model(features_a, x_a, features_b[ind], x_b, ind0, ind1)[0]
    elif features_b is None:
        prediction = model(features_a[ind], x_a, features_b, x_b, ind0, ind1)[0]
    else:
        prediction = model(features_a[ind], x_a, features_b[ind], x_b, ind0, ind1)[0]
...
def l2_loss(model, feature_a, x_a, feature_b, x_b, target, ind0, ind1):
    X = model(feature_a, x_a, feature_b, x_b, ind0, ind1)[0]
    error = jnp.sum((X - target)**2)
    return error

def batch_l2_loss(model, feature_a, x_a, feature_b, x_b, target, ind0, ind1):
    if feature_a is None:
        in_axes = (None, None, None, 0, None, 0, None, None)
    elif feature_b is None:
        in_axes = (None, 0, None, None, None, 0, None, None)
    else:
        in_axes = (None, 0, None, 0, None, 0, None, None)
    res = vmap(l2_loss, in_axes=in_axes)(model, feature_a, x_a, feature_b, x_b, target, ind0, ind1)
    return jnp.mean(res)
...
def make_step_scan(carry, n, optim, ind0, ind1):
    ...
    if features_a is None:
        loss, grads = compute_loss_and_grads(model, features_a, x_a, features_b[n], x_b, targets[n], ind0, ind1)
    elif features_b is None:
        loss, grads = compute_loss_and_grads(model, features_a[n], x_a, features_b, x_b, targets[n], ind0, ind1)
    else:
        loss, grads = compute_loss_and_grads(model, features_a[n], x_a, features_b[n], x_b, targets[n], ind0, ind1)
...
if __name__ == "__main__":
    ...
    ind0, ind1 = BiFNO_GKN_linear.get_indices(coordinates_a, coordinates_b, args['GKN_R'])
    ...
    model = BiFNO_GKN_linear.BiFNO_GKN_linear(N_layers, N_features_a, N_features_b, N_modes, D, args['GKN_proc'], args['GKN_layers'], keys[1], s1=args['s1'], s2=0, s3=args['s3'])
    ...
    for nn_ in nn:
        ...
        rel_errors = scan(compute_errors_, [model, features_a, coordinates_a, features_b, coordinates_b, targets], val_ind)[1]
    ...
    train_rel_errors = jnp.mean(scan(compute_errors_, [models[best_n], features_a, coordinates_a, features_b, coordinates_b, targets], train_ind)[1])
    val_rel_errors = jnp.mean(scan(compute_errors_, [models[best_n], features_a, coordinates_a, features_b, coordinates_b, targets], val_ind)[1])
    test_rel_errors = jnp.mean(scan(compute_errors_, [models[best_n], features_a, coordinates_a, features_b, coordinates_b, targets], test_ind)[1])
