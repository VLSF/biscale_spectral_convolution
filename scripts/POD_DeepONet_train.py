...
from architectures import POD_DeepONet
...
def POD(field):
    field_ = field.reshape(field.shape[0], -1)
    v = jnp.linalg.svd(field_, full_matrices=False)[2]
    return v

def project(field, v, N_basis):
    field_ = field.reshape(field.shape[0], -1) @ v[:N_basis].T
    return field_
...
if __name__ == "__main__":
    ...
    features_projected = project(features, features_v, args['N_basis_f'])
    trunk_params = [[features_projected.shape[1], args["N_features_trunk"], args["N_basis"]], args["N_layers_trunk"]]
    branch_params = [[coordinates.shape[0], args["N_features_branch"], args["N_basis"]], args["N_layers_branch"]]
    model = POD_DeepONet.DeepONet(trunk_params, branch_params, keys[0])
    ...
    carry = [model, features_projected, targets, coordinates, opt_state]
    make_step_scan_ = lambda a, b: POD_DeepONet.l2_make_step_scan(a, b, optim)
    ...
    for nn_ in nn:
        ...
        _, predictions = scan(POD_DeepONet.make_prediction_scan, [model, features_projected[args["N_train"]:(args["N_train"]+args["N_val"])], coordinates], jnp.arange(features_projected[args["N_train"]:(args["N_train"]+args["N_val"])].shape[0]))
    ...
    _, predictions = scan(POD_DeepONet.make_prediction_scan, [models[best_n], features_projected, coordinates], jnp.arange(features.shape[0]))
