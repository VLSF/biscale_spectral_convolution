...
from architectures import GKN_FNO
...
def compute_errors(carry, ind, ind0, ind1):
    model, features, x0, x1, targets = carry
    prediction = model(features[ind], x0, x1, ind0, ind1)
...
def l2_loss(model, feature, x0, x1, target, ind0, ind1):
    X = model(feature, x0, x1, ind0, ind1)
    error = jnp.sum((X - target)**2)
    return error

def batch_l2_loss(model, feature, x0, x1, target, ind0, ind1):
    res = vmap(l2_loss, in_axes=(None, 0, None, None, 0, None, None))(model, feature, x0, x1, target, ind0, ind1)
    return jnp.mean(res)

compute_loss_and_grads = eqx.filter_value_and_grad(batch_l2_loss)

def make_step_scan(carry, n, optim, ind0, ind1):
    model, features, x0, x1, targets, opt_state = carry
    loss, grads = compute_loss_and_grads(model, features[n], x0, x1, targets[n], ind0, ind1)
    grads = tree_map(lambda x: x.conj(), grads)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, features, x0, x1, targets, opt_state], loss
    ...
    args = {
       ...
       "-results_path": {
        ...
        "-J_in": {
            "default": 0,
            "type": int,
            "help": "subsampling rate for each spatial dimension of the input, e.g., x[:, ::2**J]"
        },
        "-J_out": {
            "default": 0,
            "type": int,
            "help": "subsampling rate for each spatial dimension of the output, e.g., x[:, ::2**J]"
        },
        "-kernel_encoder": {
            "default": 1,
            "type": int,
            "choices": [0, 1],
            "help": "if 1 kernel encoder is present, otherwise kernel decoder is present"
        },
        "-GKN_proc": {
            "default": 32,
            "type": int,
            "help": "number of features in graph kernel network"
        },
        "-GKN_layers": {
            "default": 4,
            "type": float,
...
if __name__ == "__main__":
    ...
    D = targets.ndim - 2
    targets = normalise_field(subsample_field(targets, args['J_out'], D), D)
    J1 = jnp.log2(targets.shape[-1]).astype(int).item()
    perm = random.permutation(keys[0], targets.shape[0])
    targets = targets[perm]
    
    features = normalise_field(subsample_field(data['features'], args['J_in'], D), D)[perm]
    x0 = subsample_field(np.expand_dims(data['coordinates'], 0), args['J_in'], D)[0]
    x1 = subsample_field(np.expand_dims(data['coordinates'], 0), args['J_out'], D)[0]
    J0 = jnp.log2(features.shape[-1]).astype(int).item()

    ind0, ind1 = GKN_FNO.get_indices(x0, x1, args['GKN_R'])
    ...
    N_features = [x0.shape[0] + features.shape[1], args['N_processor'], targets.shape[1]]
    N_modes = args["N_modes"]
    model = GKN_FNO.GKN_FNO(N_layers, N_features, N_modes, D, J0, J1, args['kernel_encoder'] == 1, args['GKN_proc'], args['GKN_layers'], keys[1], s1=args['s1'], s2=0, s3=args['s3'])
    ...
    carry = [model, features, x0, x1, targets, opt_state]
    make_step_scan_ = lambda a, b: make_step_scan(a, b, optim, ind0, ind1)
    compute_errors_ = lambda a, b: compute_errors(a, b, ind0, ind1)
    ...
    for nn_ in nn:
        ...
        rel_errors = scan(compute_errors_, [model, features, x0, x1, targets], val_ind)[1]
    ...
    train_rel_errors = jnp.mean(scan(compute_errors_, [models[best_n], features, x0, x1, targets], train_ind)[1])
    val_rel_errors = jnp.mean(scan(compute_errors_, [models[best_n], features, x0, x1, targets], val_ind)[1])
    test_rel_errors = jnp.mean(scan(compute_errors_, [models[best_n], features, x0, x1, targets], test_ind)[1])
