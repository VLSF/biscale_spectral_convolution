...
def l2_loss(model, x_encoder, x_decoder, x_b, target_encoder, target_decoder):
    z = model.encode(target_encoder, x_encoder, x_b)
    prediction = model.decode(z, x_decoder, x_b)
    error = jnp.sum((prediction - target_decoder)**2)
    return error

def batch_l2_loss(model, x_encoder, x_decoder, x_b, target_encoder, target_decoder):
    res = vmap(l2_loss, in_axes=[None, None, None, None, 0, 0])(model, x_encoder, x_decoder, x_b, target_encoder, target_decoder)
    return jnp.mean(res)

compute_loss_and_grads = eqx.filter_value_and_grad(batch_l2_loss)

def make_step_scan(carry, n, optim):
    model, x_encoder, x_decoder, x_b, targets_encoder, targets_decoder, opt_state = carry
    loss, grads = compute_loss_and_grads(model, x_encoder, x_decoder, x_b, targets_encoder[n], targets_decoder[n])
    grads = tree_map(lambda x: x.conj(), grads)
    updates, opt_state = optim.update(grads, opt_state, eqx.filter(model, eqx.is_array))
    model = eqx.apply_updates(model, updates)
    return [model, x_encoder, x_decoder, x_b, targets_encoder, targets_decoder, opt_state], loss
    ...
    args = {
       ...
       "-results_path": {
        ...
        "-stride": {
            "default": 2,
            "type": int,
            "help": "2*N_modes // stride points along each dimension are left after encoder"
        },
        "-kernel_size": {
            "default": 3,
            "type": int,
        ...
        "-n_codes": {
            "default": 1,
            "type": int,
            "help": "number of channels in the code"
        },
        "-J_encoder": {
            "default": 0,
            "type": int,
...
if __name__ == "__main__":
    ...
    header += ",hash,final_loss,model_size,training_time,train_error,test_error,val_error,n_basis,best_n"
    ...
    N_features = [targets.shape[1], args["N_processor"], targets.shape[1]]
    ...
    carry = [model, x_encoder, x_decoder, x_b, targets_encoder, targets_decoder, opt_state]
    ...
    for nn_ in nn:
        ...
        rel_errors = scan(compute_errors, [model, x_encoder, x_decoder, x_b, targets_encoder, targets_decoder], val_ind)[1]
    ...
    train_rel_errors = jnp.mean(scan(compute_errors, [models[best_n], x_encoder, x_decoder, x_b, targets_encoder, targets_decoder], train_ind)[1])
    val_rel_errors = jnp.mean(scan(compute_errors, [models[best_n], x_encoder, x_decoder, x_b, targets_encoder, targets_decoder], val_ind)[1])
    test_rel_errors = jnp.mean(scan(compute_errors, [models[best_n], x_encoder, x_decoder, x_b, targets_encoder, targets_decoder], test_ind)[1])
    data = "\n" + ",".join([str(args[key]) for key in args.keys()])
    data += f",{exp_hash},{history[-1]},{model_size},{training_times[best_n]},{train_rel_errors},{test_rel_errors},{val_rel_errors},{n_codes*((2*N_modes)//stride)**D},{best_n}"
