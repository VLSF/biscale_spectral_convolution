        ...
        for i in range(len(Data_)):
            ...
            if args['J_a'] != resolution_[-1]:
                ...
                for j in resolution_:
                    targets = BiFNOk_train.normalise_field(BiFNOk_train.subsample_field(targets_, j, D), D)
                    perm = random.permutation(keys[0], targets.shape[0])
                    targets = targets[perm]
                    
                    if args['features_input'] == 'a':
                        features_a = BiFNOk_train.normalise_field(BiFNOk_train.subsample_field(features_, j, D), D)[perm]
                        features_b = None
                    else:
                        features_a = None
                        features_b = BiFNOk_train.normalise_field(BiFNOk_train.subsample_field(features_, args['J_b'], D), D)[perm]

                    coordinates_a = BiFNOk_train.subsample_field(np.expand_dims(coordinates_, 0), j, D)[0]
                    if args['Nx_b'] != 0:
                        coordinates_b = BiFNOk_train.get_coordinates(args['Nx_b'], D)
                    else:
                        coordinates_b = BiFNOk_train.subsample_field(np.expand_dims(coordinates_, 0), args['J_b'], D)[0]
