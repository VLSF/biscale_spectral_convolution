...
if __name__ == "__main__":
    ...
    D = targets.ndim - 2
    targets = normalise_field(subsample_field(targets, args['J'], D), D)
    perm = random.permutation(keys[0], targets.shape[0])
    targets = targets[perm]
    
    features = normalise_field(subsample_field(data['features'], args['J'], D), D)[perm]
    coordinates = subsample_field(np.expand_dims(data['coordinates'], 0), args['J'], D)[0]
