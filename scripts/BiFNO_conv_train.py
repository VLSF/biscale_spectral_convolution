...
from architectures import BiFNO_conv
...
if __name__ == "__main__":
    ...
    J_a_ = int(jnp.log2(coordinates_a.shape[-1]).item())
    J_b_ = int(jnp.log2(coordinates_b.shape[-1]).item())
    model = BiFNO_conv.BiFNO_conv(N_layers, N_features_a, N_features_b, N_modes, D, kernel_size, keys[1], J_a_, J_b_, s1=args['s1'], s2=0, s3=args['s3'])
